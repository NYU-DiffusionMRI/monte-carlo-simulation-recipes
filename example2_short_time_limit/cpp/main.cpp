...
//
//  Created by Hong-Hsi Lee on 2/24/17.
...
#define dt 3.5e-5                // Time step in ms
    const int TN=2e4;            // Number of time steps
    const double TT=TN*dt;       // Total time
    const long NPar=5e4;         // Number of particles
    const int timepoints=1e3     // Number of time points to be recorded
    const int Tstep=TN/timepoints;
    
#define Din 2.0                 // Diffusion coefficient inside the cylinders in µm^2/ms
#define Dout 2.0                // Diffusion coefficient outside the cylinders in µm^2/s
#define Dmc 0.5                 // Diffusion coefficient in myelin, circumferential, in µm^2/s
#define kappa 0.0               // Permeability of a lipid bi-layer in µm/ms
#define n 5.0                   // Number density of lipid bi-layer in 1/µm
    const double Dmr=0.2;//kappa/n;         // Diffusivity in myelin, radial, in µm^2/s
    const double stepIN=sqrt(4*dt*Din);     // Step size in IAS in µm
    const double stepOUT=sqrt(4*dt*Dout);   // Step size in EAS in µm
    const double stepMC=sqrt(4*dt*Dmc);     // Step size in myelin in µm, circumferential
    const double stepMR=sqrt(4*dt*Dmr);     // Step size in myelin in µm, radial
    ...
    int main(int argc, char *argv[]) {
        ...
        // Voxel size/resolution of the lookup table
        ifstream myfile1 ("/examples2_short_time_limit/packing/phantom_res.txt", ios::in);
        myfile1>>res;
        myfile1.close();
        
        // Matrix size of the lookup table
        ifstream myfile2 ("/examples2_short_time_limit/packing/phantom_NPix.txt", ios::in);
        myfile2>>NPix;
        myfile2.close();
        
        // Lookup table A recording axon labels
        ...
        ifstream myfile3 ("/examples2_short_time_limit/packing/phantom_APix.txt", ios::in);
        for (i=0; i<NPix; i++){
            for (j=0; j<NPix; j++){
                myfile3>>APix[i][j];
            }
        ...
        // # Axons
        ifstream myfile4 ("/examples2_short_time_limit/packing/phantom_NAx.txt", ios::in);
        myfile4>>NAx;
        myfile4.close();
        
        // X-coordinate of the circle center
        ifstream myfile5 ("/examples2_short_time_limit/packing/phantom_xCir.txt", ios::in);
        ...
        ifstream myfile6 ("/examples2_short_time_limit/packing/phantom_yCir.txt", ios::in);
        ...
        // Circle radius
        ifstream myfile7 ("/examples2_short_time_limit/packing/phantom_rCir.txt", ios::in);
        ...
        // The smallest number > # Axons, in the base of 10
        unsigned int Nmax=0;
        ifstream myfile8 ("/examples2_short_time_limit/packing/phantom_Nmax.txt", ios::in);
        myfile8>>Nmax;
        myfile8.close();
        
        // g-ratio
        double gratio=0;
        ifstream myfile9 ("/examples2_short_time_limit/packing/phantom_gratio.txt", ios::in);
        myfile9>>gratio;
        myfile9.close();
        
        //********** Initialize Particle Positions in EAS *********
        const double probOUT=Pi/4*res*kappa/Dout;   // Probability constant for a particle from EAS to myelin
        const double probIN=Pi/4*res*kappa/Din;     // Probability constant for a particle from IAS to myelin
        const double probMY=Pi/4*res*kappa/Dmr;     // Probability constant for a particle from myelin to IAS/EAS
        double xiPar[NPar], yiPar[NPar];            // Initial positions of particles
        double xRand=0, yRand=0;                    // Random numbers
        unsigned int a=0;                           // Element of lookup table APix
        ...
        vector<int> xyParG(2,0);                    // Particle position index on the grid/lookup table
        ...
        while(k<NPar){
            ...
            // Find the axons around the particle
            xyParG=pixPosition(xyPar,NPix);
            a=APix[xyParG[0]][xyParG[1]];
            ax1=a%Nmax; ax2=a/Nmax;
            
            // If the particle is in an axon, do not take the position
        ...
        a=0; unsigned int aTmp=0;                           // Element of the lookup table APix
        ...
        vector<double> vp(2,0);                             // Step direction
        double axDis[4]={0}, axDisMin=0;                    // Distance to the axon surface and its min
        int ax_hit=0;                                       // The label of the axon hit by the particle
        ...
        for (k=0; k<NPar; k++){         // Iterate over initial positions
            
            xyi[0]=xiPar[k]; xyi[1]=yiPar[k];               // Position before a step
            xyt=xyi;                                        // Position after a step
            xytG=pixPosition(xyt, NPix);                    // Position on the grid/lookup table after a step
                        
            for (i=0; i<TN; i++){       // Iterate over time points
                
                // The axons close to the particle in previous step
                a=APix[xytG[0]][xytG[1]];
                ax1=a%Nmax, ax2=a/Nmax;
                
                // Check whether the particle is in EAS
                instruction1=false; instruction2=false;
                if ( ax1 ) {
                    xyCir[0]=xCir[ax1-1]; xyCir[1]=yCir[ax1-1];
                ...
                if ( instruction1==false & instruction2==false ) { // In EAS
                    ...
                    // Initialize velocity
                    v=JKISS(v);
                    phi=2*Pi*v.r;                               // Direction in angle phi
                    vp[0]=cos(phi); vp[1]=sin(phi);             // Step direction
                    
                    // Primitive position after the step
                    ...
                    // Check whether the segment(xyt,xyTmp) overlaps with the axon
                    for (j=0; j<4; j++) {
                        instruction[j]=false;
                        if ( ax[j] ) {
                            xyCir[0]=xCir[ax[j]-1];
                    ...
                    if (instruction[0]==false & instruction[1]==false & instruction[2]==false & instruction[3]==false ) {
                        ...
                        // Find the axon closest to the particle
                        for (j=0; j<4; j++) {
                            axDis[j]=100;
                            if ( instruction[j] ) {
                                xyCir[0]=xCir[ax[j]-1];
                    ...
                    if ( instruction1==false & instruction2==false ) { // In IAS
                        // Find the axon where the particle is staying in
                        instruction1=false; instruction2=false;
                        if ( ax1 ) {
                            xyCir[0]=xCir[ax1-1]; xyCir[1]=yCir[ax1-1];
                        ...
                        // Check whether the segment(xyt,xyTmp) overlaps with the myelin
                        xyCir[0]=xCir[ax[0]-1];
                        xyCir[1]=yCir[ax[0]-1];
                        ...
                        // Initialize the step direction, vp[0] = phi, vp[1] = randR
                        v=JKISS(v);
                        vp[0]=2*Pi*v.r;
                        ...
                        if ( inMyelin(xyTmp, xyCir, rCir[ax[0]-1], gratio) ) { // Not hitting the myelin sheath
                            ...
                            else {                  // Elastic collision from myelin to the inner myelin sheath
                            ...
                            else {                  // Elastic collision from myelin to the outer myelin sheath
                                xyCollision=elasticCollisionMyelin2EAS(xyt, stepMR/res*fabs(sin(vp[0])), xyCir, rCir[ax[0]-1]);
                                // Cancel this step if bouncing the inner sheath
                                if ( inMyelin(xyCollision, xyCir, rCir[ax[0]-1], gratio) ) {
                                    xyt=xyCollision;
                                }
                            }
        ...
        ofstream fxout("/examples2_short_time_limit/results/x_diffusion");
        ofstream fyout("/examples2_short_time_limit/results/y_diffusion");
        for (i=0; i<timepoints; i++) {
            for (k=0; k<NPar; k++) {
                if (k==NPar-1) {
        ...
        ofstream fdx2out("/examples2_short_time_limit/results/dx2_diffusion");
        ofstream fdy2out("/examples2_short_time_limit/results/dy2_diffusion");
        
        ofstream fdx4out("/examples2_short_time_limit/results/dx4_diffusion");
        ofstream fdy4out("/examples2_short_time_limit/results/dy4_diffusion");
        
        ofstream fgxSRout("/examples2_short_time_limit/results/gxSigRe_diffusion");
        ofstream fgxSIout("/examples2_short_time_limit/results/gxSigIm_diffusion");
        
        ofstream fgySRout("/examples2_short_time_limit/results/gySigRe_diffusion");
        ofstream fgySIout("/examples2_short_time_limit/results/gySigIm_diffusion");
        ...
        ofstream paraout ("/examples2_short_time_limit/results/sim_para");
