...
denom=1/4 - 2*exp(-beta1(1)^2*t/tc)/beta1(1)^2/(beta1(1)^2-1);
for i=2:numel(beta0)
    numer=numer + exp(-beta0(i)^2*t/tc)/beta0(i)^4 + exp(-beta1(i)^2*t/tc)/beta1(i)^2/(beta1(i)^2-1)*(4/beta1(i)^2-3/2) +...
        exp(-beta2(i)^2*t/tc)/beta2(i)^2/(beta2(i)^2-4)/2;
    denom=denom - 2*exp(-beta1(i)^2*t/tc)/beta1(i)^2/(beta1(i)^2-1);
end
numer=6*numer;
denom=(denom).^2;
RK=numer./denom-3;
